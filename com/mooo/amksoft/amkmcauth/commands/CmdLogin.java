...
public class CmdLogin implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args)
    {
        if (args.length < 1) {
  		...
  		String rawPassword = AmkAUtils.getFinalArg(args, 0).trim(); // support spaces
    ...
    private static boolean LogMeIn(Player cs, String cmd, String rawPassword) {
    	if (cmd.equalsIgnoreCase("login")) {
            ...
            String SavedPassword = ap.getPasswordHash(); // Old/Saved password of player. 

            if (!hashedPassword.equals(SavedPassword) && !Config.MySqlDbHost.equals("")) {
    	        ...
    	        try {
    		        ...
    		        //Code using ResultSet entries here
    		       	if (res.next() == true) { 
    		       		// We have a record, Tell AmkMcAuth THIS is the corret password.
                ...
                for (String playerAction : Config.playerActionLogin) {
        			if(!playerAction.trim().isEmpty()) {
        	        	...
        	        	try {
        	        		...
        	        		AmkMcAuth.getInstance().getLogger().info("Error login Executing: " + playerAction.replace("$P", ap.getUserName()) );
